...
//This function - genProbs -takes a given day's globalInput, and a day's personalised configuration input (data for each of 4 fractals)
...
var genProbs = function(globalInput, confInput){
    ...
    for (var i = 0; i < 5; i++) {
        ...
        var temp_frac_ind = []
        //loop through trials 
        for (var j = 0; j < globalInput.length; j++){
            //loop through fractals 
            for (var k = 0; k < globalInput[j].length; k++){
                //if fractal is present in global config add a 1 to array, and keep index 
                if (globalInput[j][k][0] == i){
                    //this is an array representing whether to present the magnitude or not 
                    temp_arr.push(0)
                    //this is an index of where this item is in the global configuration
                    temp_trial_ind.push(j)
                    //this is an index of which fractal in a trial this item is 
                    temp_frac_ind.push(k)
        ...
        for (var l = 0; l < proportion; l++){ temp_arr[l] = 1}
    ...
    for (var i = 0; i <5; i++) {
        ...
        for (var m = 0; m < proportion; m++){
            //replace each 0 with magnitude up until the correct proportion 
            temp_five[m] = fractal.magnitudeConfig
        }
        //shuffle 
